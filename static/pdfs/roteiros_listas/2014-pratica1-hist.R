...
PlotHistograms = function(file_url, cor){
    ...
    ggplot(m_cenario, aes(value, group = variable)) + geom_histogram(fill=cor, binwidth = 0.5)+ facet_wrap(~variable) + theme_bw()+ xlab("Numero de Azuis")+ ylab("Frequencia")+ labs(title=names(cenario_df)[1])
}
